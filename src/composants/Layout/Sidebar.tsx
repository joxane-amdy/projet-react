...
export default function Sidebar({ mobileOpen = false, onMobileClose }: SidebarProps) {
  ...
  const navigate_ = (path: string) => {
    navigate(path);
    onMobileClose?.(); // ferme la sidebar sur mobile après navigation
  };
  ...
  const sidebarContent = (
    <aside
      className={[
        ...
        // Desktop : sidebar fixe avec collapse
        "lg:flex-shrink-0",
        collapsed ? "lg:w-16" : "lg:w-56",
        // Mobile : toujours pleine largeur dans l'overlay
        "w-64",
      ].join(" ")}
    >
      {/* Logo + bouton collapse (desktop) / fermer (mobile) */}
      <div className="flex items-center justify-between px-4 py-5 border-b border-white/10">
        ...
        <button
          ...
          className="lg:hidden text-white/60 hover:text-white transition-colors ml-auto"
        >
        ...
        <button
          ...
          className="hidden lg:block text-white/60 hover:text-white transition-colors ml-auto"
        >
          {collapsed ? (
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
