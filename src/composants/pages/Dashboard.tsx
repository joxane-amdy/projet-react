...
export default function Dashboard() {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      {/* ── SIDEBAR ── */}
      <Sidebar onNouvellesTaches={ouvrirAjout} />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="bg-emerald-700 px-6 py-3.5 flex items-center justify-between flex-shrink-0">
          <div>
            <h1 className="text-white font-semibold text-base">Tableau de bord</h1>
          ...
          <button
            onClick={ouvrirAjout}
            className="flex items-center gap-2 bg-white text-emerald-700 font-semibold text-sm px-4 py-2 rounded-xl hover:bg-emerald-50 transition-colors shadow-sm"
          >
        ...
        <main className="flex-1 px-6 py-6 space-y-6 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              <div key={label} className="bg-white rounded-2xl border border-gray-100 px-5 py-4 flex items-start gap-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              <p className="text-sm font-semibold text-gray-700 mb-1">Tâches par type</p>
              <p className="text-xs text-gray-400 mb-4">Répartition selon la catégorie</p>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              <p className="text-sm font-semibold text-gray-700 mb-1">Tâches par priorité</p>
              <p className="text-xs text-gray-400 mb-4">Haute, normale, basse</p>
              <div style={{ height: 200 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={donutData} cx="40%" cy="50%" innerRadius={52} outerRadius={80} paddingAngle={2} dataKey="value">
                    ...
                    />
                    <Tooltip formatter={(value, name) => [value ?? 0, name]} contentStyle={{ fontSize: 12, borderRadius: 8, border: "1px solid #e5e7eb", boxShadow: "none" }} />
                  </PieChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
                ...
                {FILTRES.map((filtre) => (
                  <button
                    ...
                    className={[
                      ...
                      filtreActif === filtre ? "bg-white text-gray-800 shadow-sm" : "text-gray-500 hover:text-gray-700",
                    ].join(" ")}
                  >
                    {filtre}
                  </button>
            ...
            {chargement ? (
              ...
              <div className="text-center py-12 text-gray-400">
                ...
                <p className="text-sm mb-1">{filtreActif === "Toutes" ? "Aucune tâche pour le moment" : `Aucune tâche "${filtreActif}"`}</p>
                {formMode === null && (
                  <button onClick={ouvrirAjout} className="mt-3 px-4 py-2 text-xs bg-emerald-600 text-white rounded-xl font-semibold hover:bg-emerald-700 transition-colors">
                    + Ajouter une tâche
                  </button>
                )}
              </div>
